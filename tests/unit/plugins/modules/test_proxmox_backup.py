...
MINIMAL_PERMISSIONS = {
    ...
    "/storage/local-zfs": {"Datastore.AllocateSpace": 1, "Datastore.Audit": 1},
...
NODES = [
    {"level": "", "type": "node", "node": "node1", "status": "online", "id": "node/node1", "cgroup-mode": 2},
    {"status": "online", "id": "node/node2", "cgroup-mode": 2, "level": "", "node": "node2", "type": "node"},
    {"status": "online", "id": "node/node3", "cgroup-mode": 2, "level": "", "node": "node3", "type": "node"},
]
...
TASKLOG_API_RETURN = {
    "node1": [
        ...
        {"t": "INFO: Starting Backup of VM 100 (lxc)", "n": 2},
        {"n": 23, "t": "INFO: adding notes to backup"},
        {"n": 24, "t": "INFO: Finished Backup of VM 100 (00:00:03)"},
        {"n": 25, "t": "INFO: Backup finished at 2024-11-25 16:28:03"},
        {"t": "INFO: Backup job finished successfully", "n": 26},
        {"n": 27, "t": "TASK OK"},
    ],
    "node2": [
        {
        ...
        {"t": "INFO: Starting Backup of VM 101 (kvm)", "n": 2},
        {"n": 24, "t": "INFO: Finished Backup of VM 100 (00:00:03)"},
        {"n": 25, "t": "INFO: Backup finished at 2024-11-25 16:28:03"},
        {"t": "INFO: Backup job finished successfully", "n": 26},
        {"n": 27, "t": "TASK OK"},
...
class TestProxmoxBackup(ModuleTestCase):
    def setUp(self):
        ...
        self.mock_get_permissions = patch.object(proxmox_backup.ProxmoxBackupAnsible, "_get_permissions").start()
        self.mock_get_storages = patch.object(proxmox_utils.ProxmoxAnsible, "get_storages").start()
        self.mock_get_resources = patch.object(proxmox_backup.ProxmoxBackupAnsible, "_get_resources").start()
        self.mock_get_tasklog = patch.object(proxmox_backup.ProxmoxBackupAnsible, "_get_tasklog").start()
        self.mock_post_vzdump = patch.object(proxmox_backup.ProxmoxBackupAnsible, "_post_vzdump").start()
        self.mock_get_taskok = patch.object(proxmox_backup.ProxmoxBackupAnsible, "_get_taskok").start()
    ...
    def test_fail_insufficient_permissions(self):
        ...
        assert (
            result["msg"]
            == "Insufficient permission: Performance_tweaks and bandwidth require 'Sys.Modify' permission for '/'"
        )
        assert self.mock_get_taskok.call_count == 0
        assert self.mock_post_vzdump.call_count == 0
