...
from ansible_collections.community.proxmox.plugins.connection.proxmox_qm_remote import authenticity_msg, MyAddPolicy
...
@patch('paramiko.SSHClient')
def test_exec_command_qm_not_found(mock_ssh, connection):
    """ Test command execution when qm is not found """
    mock_client = MagicMock()
    mock_ssh.return_value = mock_client
    mock_channel = MagicMock()
    mock_transport = MagicMock()
    ...
    mock_channel.makefile_stderr.return_value = [to_bytes('qm: not found')]

    connection._connected = True
    connection.ssh = mock_client

    with pytest.raises(AnsibleError, match='qm not found in path of host'):
...
def test_close_with_lock_file(connection):
    ...
    connection._connected = True
    connection.keyfile = '/tmp/qm-remote-known_hosts-test'
    connection.set_option('host_key_checking', True)
