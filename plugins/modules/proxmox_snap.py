...
def validate_params(params):
    ...
    if unbind is True:
        ...
        if api_user != "root@pam" or not api_password:
            raise AnsibleError("Parameter 'unbind=True' requires 'api_user' and 'api_password' options.")
...
class ProxmoxSnapAnsible(ProxmoxAnsible):
        ...
        while timeout >= 0:
            ...
            if timeout == 0:
                self.module.fail_json(
                    msg="Reached timeout while waiting for VM to start. Last line in task before timeout: %s"
    ...
    def shutdown_instance(self, vm, vmid, timeout):
        ...
        while timeout >= 0:
            ...
            if timeout == 0:
                self.module.fail_json(
                    msg="Reached timeout while waiting for VM to stop. Last line in task before timeout: %s"
                    % self.proxmox_api.nodes(vm["node"]).tasks(taskid).log.get()[:1]
                )
            time.sleep(1)
        return False
