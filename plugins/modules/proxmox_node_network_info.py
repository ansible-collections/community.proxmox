...
options:
  ...
  check_changes:
    description:
      ...
      - When enabled, the module will also return information about any staged changes that are waiting to be applied.
...
- name: Get all OVS bridge interfaces on a node
  community.proxmox.proxmox_node_network_info:
    api_host: proxmox.example.com
    api_user: root@pam
    api_password: secret
...
- name: Get all network interfaces and check for pending changes
  community.proxmox.proxmox_node_network_info:
    api_host: proxmox.example.com
    api_user: root@pam
    api_password: secret
...
- name: Check only for pending changes (without getting interface details)
  community.proxmox.proxmox_node_network_info:
    api_host: proxmox.example.com
    api_user: root@pam
    api_password: secret
...
pending_changes:
  description: Pending network configuration changes (only returned when C(check_changes=true))
  returned: when check_changes is true
  type: str
  sample: |
    ...
    +        bridge_ports eth1
has_pending_changes:
  description: Whether there are any pending network configuration changes (only returned when C(check_changes=true))
  returned: when check_changes is true
  type: bool
  sample: true
"""
