...
options:
  node_name:
    description:
      - The targeted Proxmox VE node to perform actions on.
    type: str
    required: true
  nodes:
    description:
      - A list of Proxmox VE nodes where the storage action should be performed on.
    type: list
    elements: str
    required: true
  name:
...
class ProxmoxNodeAnsible(ProxmoxAnsible):
    ...
    def add_storage(self):
        ...
        if storage_type == "cephfs":
            ...
            if not all([monhost, path]):
                self.module.fail_json(msg="CephFS storage requires 'monhost' and 'path' parameter.")
            else:
...
def main():
    ...
    storage_args = dict(
        ...
        content=dict(type='list', elements='str', required=True, choices=["images", "snippets", "import", "iso", "backup", "rootdir", "vztmpl"]),
    )
