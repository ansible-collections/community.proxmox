...
class Connection(ConnectionBase):
    ...
    def _build_pct_command(self, cmd: str) -> str:
        ...
        if self.get_option("remote_user") != "root":
            ...
            display.vvv(
                f"INFO Running as non root user: {self.get_option('remote_user')}, trying to run pct with become method: "
    ...
    def exec_command(self, cmd: str, in_data: bytes | None = None, sudoable: bool = True) -> tuple[int, bytes, bytes]:
        ...
        if "pct: not found" in stderr.decode("utf-8"):
            raise AnsibleError(f"pct not found in path of host: {to_text(self.get_option('remote_addr'))}")
        ...
        try:
            with open(in_path, "rb") as f:
                ...
                returncode, stdout, stderr = self.exec_command(
                    " ".join([self._shell.executable, "-c", self._shell.quote(f"cat > {out_path}")]),
                    in_data=data,
                    sudoable=False,
                )
            if returncode != 0:
    ...
    def fetch_file(self, in_path: str, out_path: str) -> None:
        ...
        try:
            returncode, stdout, stderr = self.exec_command(
                " ".join([self._shell.executable, "-c", self._shell.quote(f"cat {in_path}")]), sudoable=False
            )
            if returncode != 0:
                if "cat: not found" in stderr.decode("utf-8"):
                    raise AnsibleError(f"cat not found in path of container: {to_text(self.get_option('vmid'))}")
